...
public class card_physics : MonoBehaviour
{
    public bool collide = false;
    public float distance;
    public bool targeted = false;
    public Vector3 startingX;
    public bool card_move = false;
    Vector3 offset;
    ...
    void Collision_Handler(float dt)
    {
        ...
        for (int i = 0; i < vertices.Length; i++)
        {
            ...
            if (Vector3.Dot(x_i, N) < 0 && Vector3.Dot(v_i, N) < 0)
            {
        ...
        if (count != 0)
        {
            ...
            {
                restitution = .6f; // keep oscillation
            }
    ...
    void Update()
    {
        ...
        if (!card_move)
        {
            ...
            Quaternion wQuat = new Quaternion(w.x, w.y, w.z, 0); // Create quaternion from current w
            Quaternion prodQuat = Quaternion_Multiply(wQuat, q); // Create product of the current q and this new quaternion

            float integrate = dt / 2; // integration

            prodQuat.x = prodQuat.x * integrate;
            prodQuat.y = prodQuat.y * integrate;
            prodQuat.z = prodQuat.z * integrate;
            prodQuat.w = prodQuat.w * integrate; // integrate the product from above
            q.x = q.x + prodQuat.x;
            q.y = q.y + prodQuat.y;
            q.z = q.z + prodQuat.z;
            q.w = q.w + prodQuat.w;
